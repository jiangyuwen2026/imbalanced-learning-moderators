...
# 创建结果目录
os.makedirs('Results/ModerateImbalance', exist_ok=True)
...
def run_experiment():
    ...
    for ds_idx, (name, data_id, expected_ir) in enumerate(MODERATE_DATASETS, 1):
        print(f"\n[{ds_idx}/{len(MODERATE_DATASETS)}] 数据集: {name}")
    ...
    print("\n" + "=" * 70)
    print("实验完成!")
    print(f"结果保存: Results/ModerateImbalance/moderate_results.csv")
